...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="glass-card p-8 rounded-3xl flex items-center gap-6 border-white/[0.05] hover:bg-white/[0.05] transition-all cursor-default group">
    ...
    <div className="flex flex-col gap-10 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div>
          ...
          <h2 className="text-5xl md:text-6xl font-serif font-bold tracking-tighter text-white">
          ...
          <p className="text-textMuted mt-4 text-lg max-w-xl leading-relaxed">
            Your creative output has increased by <span className="text-white font-bold">12%</span> this week. Ready for the next masterpiece?
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
          ...
          <Link to="/creator-lab" className="bg-white/[0.03] border border-white/[0.08] hover:bg-white/[0.05] px-10 py-5 flex items-center justify-center gap-3 text-lg font-bold rounded-2xl transition-all flex-1 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="flex items-center justify-between">
             <h3 className="text-2xl font-serif font-bold text-white tracking-tight">Recent Studio Sessions</h3>
            ...
            {recentDocs.length > 0 ? (
              recentDocs.map((doc, i) => (
                <motion.div
                  ...
                  className="glass-panel p-6 rounded-3xl flex items-center justify-between group cursor-pointer border-white/[0.05] hover:bg-white/[0.05] transition-all"
                >
                  <div className="flex items-center gap-6">
                     <div className="w-12 h-12 rounded-xl bg-white/[0.03] border border-white/[0.08] flex items-center justify-center text-textMuted group-hover:text-primary transition-colors">
                        <FileText size={20} />
                     </div>
                     <div>
                        <h4 className="font-bold text-white line-clamp-1 group-hover:text-primary transition-colors">{doc.title}</h4>
                  ...
                  <div className="flex items-center gap-8">
                     ...
                     <div className="w-10 h-10 rounded-full bg-white/[0.03] flex items-center justify-center text-textMuted group-hover:text-white transition-all transform group-hover:translate-x-1">
                        <ArrowRight size={18} />
                     </div>
                  </div>
                </motion.div>
                ...
                <div className="glass-panel p-12 rounded-[40px] text-center border-white/[0.05]">
        ...
        <div className="flex flex-col gap-6">
           <h3 className="text-2xl font-serif font-bold text-white tracking-tight">Studio Insights</h3>
           <div className="glass-panel bg-gradient-to-br from-primary/10 to-transparent rounded-[40px] p-8 border-white/[0.08] relative overflow-hidden group">
              ...
              <div className="relative z-10 text-center flex flex-col items-center">
                 <div className="w-16 h-16 rounded-2xl bg-white/[0.05] border border-white/[0.1] flex items-center justify-center shadow-2xl mb-8">
                    <Star size={28} className="text-primary animate-pulse" />
                 </div>
                 <h4 className="font-serif font-bold text-2xl mb-4 text-white">Writing Momentum</h4>
                 <p className="text-sm text-textMuted leading-relaxed mb-8">
                    "Consistency is the key to mastery. You've processed <span className="text-white font-bold">12 documents</span> this month—keep the flow alive."
                 </p>
                 <Link to="/stats" className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] hover:text-white transition-colors">
                    Detailed Report
                 </Link>
              </div>
           </div>

           <div className="glass-panel p-8 rounded-[40px] border-white/[0.05]">
              <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-textMuted mb-8 border-b border-white/[0.05] pb-4">Studio Protocol</h4>
              <ul className="space-y-6">
                 {[
                   ...
                   <li key={i} className="flex gap-4 text-xs font-medium text-textMuted leading-relaxed hover:text-white transition-colors cursor-default group">
